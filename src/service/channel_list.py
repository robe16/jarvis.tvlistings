from log.log import log_internal
from resources.global_resources.log_vars import logException
from resources.lang.enGB.logs import *


def get_channel_item(chan_name):
    try:
        return channels[chan_name]
    except Exception as e:
        log_internal(logException,
                     logDesc_channel_functions_getItem,
                     description='channel "{chan_name}"'.format(chan_name=chan_name),
                     exception=e)
        return False


def get_channel_item_listingsrc_list(chan_name):
    try:
        chan = get_channel_item(chan_name)
        return chan.keys()
        # return channels['channels'][chan_name]['sources']
    except Exception as e:
        log_internal(logException,
                     logDesc_channel_functions_getSources,
                     description='channel "{chan_name}"'.format(chan_name=chan_name),
                     exception=e)
        return False


def get_channel_item_listingsrc(chan_name, src):
    try:
        item = get_channel_item(chan_name)
        if bool(item):
            return item[src]
        return False
    except Exception as e:
        log_internal(logException,
                     logDesc_channel_functions_getSource_details,
                     description='channel "{chan_name}" and source "{src}"'.format(chan_name=chan_name,
                                                                                   src=src),
                     exception=e)
        return False


def get_channel_item_listingsrc_id(chan_name, src):
    try:
        chan = get_channel_item_listingsrc(chan_name, src)
        return chan['id']
    except Exception as e:
        log_internal(logException,
                     logDesc_channel_functions_getSource_id,
                     description='channel "{chan_name}" and source "{src}"'.format(chan_name=chan_name,
                                                                                   src=src),
                     exception=e)
        return False


def get_channel_item_listingsrc_offset(chan_name, src):
    try:
        chan = get_channel_item_listingsrc(chan_name, src)
        return chan['offset']
    except Exception as e:
        log_internal(logException,
                     logDesc_channel_functions_getSource_offset,
                     description='channel "{chan_name}" and source "{src}"'.format(chan_name=chan_name,
                                                                                   src=src),
                     exception=e)
        return False


channels = {
    "BBC ONE": {
        "bleb": {"id": "bbc1",
                 "offset": 0}
    },
    "BBC TWO": {
        "bleb": {"id": "bbc2",
                 "offset": 0}
    },
    "ITV": {
        "bleb": {"id": "p_itv1",
                 "offset": 0}
    },
    "Channel 4": {
        "bleb": {"id": "ch4",
                 "offset": 0}
    },
    "Channel 4 +1": {
        "bleb": {"id": "ch4",
                 "offset": 1}
    },
    "Channel 5": {
        "bleb": {"id": "five",
                 "offset": 0}
    },
    "Channel 5 +1": {
        "bleb": {"id": "five",
                 "offset": 1}
    },
    "My5": {},
    "BBC FOUR": {
        "bleb": {"id": "bbc4",
                 "offset": 0}
    },
    "Sky1": {
        "bleb": {"id": "sky_one",
                 "offset": 0}
    },
    "Sky2": {
        "bleb": {"id": "sky_two",
                 "offset": 0}
    },
    "Sky Living": {
        "bleb": {"id": "living_tv",
                 "offset": 0}
    },
    "Sky Living +1": {
        "bleb": {"id": "living_tv",
                 "offset": 1}
    },
    "ITV +1": {
        "bleb": {"id": "p_itv1",
                 "offset": 1}
    },
    "ITV2": {
        "bleb": {"id": "p_itv2",
                 "offset": 0}
    },
    "ITV2 +1": {
        "bleb": {"id": "p_itv2",
                 "offset": 1}
    },
    "ITV3": {
        "bleb": {"id": "p_itv3",
                 "offset": 0}
    },
    "ITV3 +1": {
        "bleb": {"id": "p_itv3",
                 "offset": 1}
    },
    "ITV4": {
        "bleb": {"id": "p_itv4",
                 "offset": 0}
    },
    "ITV4 +1": {
        "bleb": {"id": "p_itv4",
                 "offset": 1}
    },
    "ITVBe": {},
    "ITVBe +1": {},
    "Sky Arts": {},
    "Pick": {},
    "W": {
        "bleb": {"id": "watch",
                 "offset": 0}
    },
    "W +1": {
        "bleb": {"id": "watch",
                 "offset": 1}
    },
    "Gold": {
        "bleb": {"id": "uk_gold",
                 "offset": 0}
    },
    "Gold +1": {
        "bleb": {"id": "uk_gold",
                 "offset": 1}
    },
    "Dave": {
        "bleb": {"id": "dave",
                 "offset": 0}
    },
    "Dave ja vu": {
        "bleb": {"id": "dave",
                 "offset": 1}
    },
    "Alibi": {},
    "Alibi +1": {},
    "Comedy Central": {
        "bleb": {"id": "paramount",
                 "offset": 0}
    },
    "Comedy Central +1": {
        "bleb": {"id": "paramount",
                 "offset": 1}
    },
    "Comedy Central Extra": {
        "bleb": {"id": "paramount2",
                 "offset": 0}
    },
    "MTV": {
        "bleb": {"id": "mtv",
                 "offset": 0}
    },
    "syfy": {
        "bleb": {"id": "scifi",
                 "offset": 0}
    },
    "syfy +1": {
        "bleb": {"id": "scifi",
                 "offset": 1}
    },
    "Universal Channel": {},
    "Universal Channel +1": {},
    "Challenge": {
        "bleb": {"id": "challenge",
                 "offset": 0}
    },
    "Challenge +1": {
        "bleb": {"id": "challenge",
                 "offset": 1}
    },
    "E4": {
        "bleb": {"id": "e4",
                 "offset": 0}
    },
    "E4 +1": {
        "bleb": {"id": "e4",
                 "offset": 1}
    },
    "More4": {
        "bleb": {"id": "more4",
                 "offset": 0}
    },
    "More4 +1": {
        "bleb": {"id": "more4",
                 "offset": 1}
    },
    "CBS Reality": {},
    "CBS Reality +1": {},
    "Horror Channel": {},
    "Horror Channel +1": {},
    "5*": {
        "bleb": {"id": "fiver",
                 "offset": 0}
    },
    "5* +1": {
        "bleb": {"id": "fiver",
                 "offset": 1}
    },
    "5 USA": {},
    "5 USA +1": {},
    "5 Spike": {},
    "E! Entertainment": {},
    "FOX": {
        "bleb": {"id": "fx",
                 "offset": 0}
    },
    "FOX +1": {
        "bleb": {"id": "fx",
                 "offset": 1}
    },
    "Made in Leeds": {},
    "Real Lives": {},
    "Real Lives +1": {},
    "BBC ALBA": {},
    "S4C": {
        "bleb": {"id": "s4c",
                 "offset": 0}
    },
    "TLC": {},
    "TLC +1": {},
    "DMAX": {},
    "DMAX +1": {},
    "Quest": {},
    "Quest +1": {},
    "BET": {},
    "True Entertainment": {},
    "Drama": {},
    "CBS Action": {},
    "Sony TV": {},
    "4Seven": {},
    "CBS Drama": {},
    "TruTV": {},
    "Sky Cinema Premiere": {
        "bleb": {"id": "sky_movies_premiere",
                 "offset": 0}
    },
    "Sky Cinema Premiere +1": {
        "bleb": {"id": "sky_movies_premiere",
                 "offset": 1}
    },
    "Sky Cinema Hits": {
        "bleb": {"id": "sky_movies_showcase",
                 "offset": 0}
    },
    "Sky Cinema Greats": {
        "bleb": {"id": "sky_movies_modern_greats",
                 "offset": 0}
    },
    "Sky Cinema Disney": {},
    "Sky Cinema Family": {
        "bleb": {"id": "sky_movies_family",
                 "offset": 0}
    },
    "Sky Cinema Action": {
        "bleb": {"id": "sky_movies_action_thriller",
                 "offset": 0}
    },
    "Sky Cinema Comedy": {
        "bleb": {"id": "sky_movies_comedy",
                 "offset": 0}
    },
    "Sky Cinema Thriller": {
        "bleb": {"id": "sky_movies_crime_thriller",
                 "offset": 0}
    },
    "Sky Cinema Drama": {
        "bleb": {"id": "sky_movies_drama",
                 "offset": 0}
    },
    "Sky Cinema Sci Fi & Horror": {
        "bleb": {"id": "sky_movies_sci-fi_horror",
                 "offset": 0}
    },
    "Sky Cinema Select": {
        "bleb": {"id": "sky_movies_indie",
                 "offset": 0}
    },
    "TCM": {
        "bleb": {"id": "tcm",
                 "offset": 0}
    },
    "TCM +1": {
        "bleb": {"id": "tcm",
                 "offset": 1}
    },
    "Movies 24": {},
    "Movies 24 +1": {},
    "True Movies": {},
    "Sony Movie Channel": {},
    "Film4": {
        "bleb": {"id": "film_four",
                 "offset": 0}
    },
    "Film4 +1": {
        "bleb": {"id": "film_four",
                 "offset": 1}
    },
    "Sky Sports News": {
        "bleb": {"id": "sky_sports_news",
                 "offset": 0}
    },
    "Sky Sports Mix": {},
    "Sky Sports Main Event": {},
    "Sky Sports Premier League": {},
    "Sky Sports Football": {},
    "Sky Sports Cricket": {},
    "Sky Sports Golf": {},
    "Sky Sports F1": {},
    "Sky Sports Action": {},
    "Sky Sports Arena": {},
    "At The Races": {},
    "BT Sport 1": {},
    "BT Sport 2": {},
    "BT Sport 3": {},
    "BT Sport ESPN": {},
    "Eurosport 1": {
        "bleb": {"id": "british_eurosport",
                 "offset": 0}
    },
    "Eurosport 2": {},
    "Motorsport TV": {},
    "FIA Formula E": {},
    "BIKE": {},
    "COPA90": {},
    "Yesterday": {},
    "Yesterday +1": {},
    "Eden": {},
    "Eden +1": {},
    "Discovery Channel": {},
    "Discovery Channel +1": {},
    "Investigation Discovery": {},
    "Investigation Discovery +1": {},
    "Animal Planet": {},
    "Animal Planet +1": {},
    "Discovery Science": {},
    "Discovery Science +1": {},
    "Discovery Turbo": {},
    "Discovery History": {},
    "Discovery History +1": {},
    "Nat Geo Wild": {},
    "National Geographic": {},
    "National Geographic +1": {},
    "Community Channel": {},
    "History": {},
    "History +1": {},
    "H2": {},
    "Lifetime": {},
    "Crime and Investigation": {},
    "PBS America": {},
    "Forces TV": {},
    "Good Food": {},
    "Good Food +1": {},
    "Food Network": {},
    "Home": {
        "bleb": {"id": "uktv_style",
                 "offset": 0}
    },
    "Home +1": {
        "bleb": {"id": "uktv_style",
                 "offset": 1}
    },
    "Really": {},
    "Discovery Home & Health": {},
    "Discovery Home & Health +1": {},
    "Discovery Shed": {},
    "Ginx Esports TV": {},
    "Travel Channel": {},
    "CBBC": {
        "bleb": {"id": "cbbc",
                 "offset": 0}
    },
    "Cbeebies": {
        "bleb": {"id": "cbeebies",
                 "offset": 0}
    },
    "Cartoon Network": {
        "bleb": {"id": "cartoon_network",
                 "offset": 0}
    },
    "Cartoon Network +1": {
        "bleb": {"id": "cartoon_network",
                 "offset": 1}
    },
    "Cartoonito": {},
    "Disney XD": {},
    "Disney XD +1": {},
    "Nickelodeon": {
        "bleb": {"id": "nickelodeon",
                 "offset": 0}
    },
    "Nickelodeon +1": {
        "bleb": {"id": "nickelodeon",
                 "offset": 1}
    },
    "Nick Junior": {},
    "Nick Junior Too": {},
    "Nick Toons": {},
    "Disney Channel": {
        "bleb": {"id": "disney",
                 "offset": 0}
    },
    "Disney Channel +1": {
        "bleb": {"id": "disney",
                 "offset": 1}
    },
    "Disney Junior": {},
    "Boomerang": {},
    "Boomerang +1": {},
    "CITV": {
        "bleb": {"id": "citv",
                 "offset": 0}
    },
    "Pop": {},
    "Tiny Pop": {},
    "MTV Music": {},
    "MTV Live HD": {},
    "MTV Hits": {},
    "MTV Base": {},
    "MTV Dance": {},
    "MTV Rocks": {},
    "VH1": {
        "bleb": {"id": "vh1",
                 "offset": 0}
    },
    "Viva": {},
    "4 Music": {
        "bleb": {"id": "4music",
                 "offset": 0}
    },
    "The Box": {},
    "Kiss": {},
    "Box Hits": {},
    "Box Upfront": {},
    "Magic": {},
    "Kerrang! TV": {},
    "Vintage TV": {},
    "Clubland": {},
    "BBC News": {
        "bleb": {"id": "bbc_news24",
                 "offset": 0}
    },
    "Sky News": {
        "bleb": {"id": "sky_news",
                 "offset": 0}
    },
    "BBC Parliament": {
        "bleb": {"id": "bbc_parliament",
                 "offset": 0}
    },
    "CNN": {},
    "Bloomberg": {
        "bleb": {"id": "boomerang",
                 "offset": 0}
    },
    "CNBC": {},
    "Euronews": {},
    "NDTV 24x7": {},
    "BBC Radio 1": {},
    "BBC Radio 1 Xtra": {},
    "BBC Radio 2": {},
    "BBC Radio 3": {},
    "BBC Radio 4 FM": {},
    "BBC Radio 4 Extra": {},
    "BBC Radio 4 Long Wave": {},
    "BBC Radio 5 Live": {},
    "BBC Radio 5 Live Sports Extra": {},
    "BBC Radio 6 Music": {},
    "BBC World Service": {},
    "Absolute Radio": {},
    "Smooth Radio": {},
    "Heart FM": {},
    "LBC": {},
    "Classic FM": {},
    "Gold Radio": {},
    "Planet Rock": {},
    "Magic Radio": {},
    "Absolute 80s": {},
    "Absolute Classic Rock": {},
    "Capital FM": {},
    "Capital Xtra": {},
    "Radio X": {},
    "Kiss FM": {},
    "talkSPORT": {
        "sources": {}
    }
}
